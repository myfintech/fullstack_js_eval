...
describe("People API", () => {
  ...
  it("GET /v1/people/:personID/addresses/:addressID should return an address by its id and its person_id", async () => {
    await client
      .get(
        `/v1/people/${fixtures.firstPerson.id}/addresses/${fixtures.firstAddress.person_id}`
      )
      .expect(httpStatusCodes.OK)
      .then((resp) => {
        expect(resp["body"]["line1"]).to.equal(fixtures.firstAddress["line1"]);
        expect(resp["body"]["person_id"]).to.equal(
          fixtures.firstAddress["person_id"]
        );
      });
  });
